...
select p.nombre from pokemon p join pokemon_tipo t 
	on p.numero_pokedex = t.numero_pokedex 
		where t.id_tipo = (select id_tipo from tipo where	
			nombre like 'agua%');
            
select p.nombre 'Pokemon', ti.nombre 'Tipo' from pokemon_tipo t
	...
	inner join pokemon p on t.numero_pokedex = p.numero_pokedex
		inner join tipo ti 
			on ti.id_tipo = t.id_tipo
				where ti.nombre = 'fuego' and ti.nombre = 'volador';

-- 6. Mostrar los pokemon con una estadística base de ps mayor que 200.
select nombre, ps from pokemon p
...
select nombre, peso, altura, e.pokemon_origen from pokemon p
...
select p.nombre, p.altura, p.peso from pokemon p, evoluciona_de ev
	where p.numero_pokedex = ev.pokemon_origen 
		and ev.pokemon_evolucionado = (select numero_pokedex from pokemon where lower(nombre) = 'arbok');
        
-- 8. Mostrar aquellos pokemon que evolucionan por intercambio.
select pk.nombre, tev.tipo_evolucion from forma_evolucion fev 
...
-- create view
-- union_pokemon as 
-- select pk.nombre_pokedex, pk.nombre, pk.peso, pk.altura, tev.tipo_evolucion
-- 	from foforma_evolucion fev inner join tipo_evolucion tev
-- 		on (tev.id_tipo_evolucion = fev.tipo_evolucion)
-- 			inner join pokemon_forma_evolucion pfev
-- 				on (pfev.id_forma_evolucion = fev.tipo_evolucion)
-- 					inner join pokemon pk 
-- 						on (pk.numero_pokedex = pfev.numero_pokedex);
--                         
...
select count(t.id_tipo) 'Nro de Movimientos', t.nombre 'Movimiento tipo'
	from movimiento m join tipo t 
		on t.id_tipo = m.id_tipo 
			group by m.id_tipo;

-- 13. Mostrar todos los movimientos que puedan envenenar.
select m.*, mes.probabilidad, es.efecto_secundario from movimiento_efecto_secundario mes
	join efecto_secundario es on es.efecto_secundario = mes.id_efecto_secundario
		join movimiento m
			on mes.id_movimiento = m.id_movimiento
				where mes.id_efecto_secundario = (select id_efecto_secundario from efecto_secundario
					where efecto_secundario like 'env%');
